...
behaviors:
  ...
  - behavior_id: BEH-310
    title: "a dimension put back to proposed keeps its last confirmed value: no change, no entry, snapshot untouched"
    state: accepted
    level: unit
    adapter: pytest
    locator: tests/test_rescan.py::test_re_proposing_a_confirmed_answer_neither_moves_the_snapshot_nor_files_an_entry
  - behavior_id: BEH-037
    title: "a profile with an unanswered dimension returns exit 2 with the problems and writes no snapshot"
    state: accepted
    level: unit
    adapter: pytest
...
## What
`rescan.run(cdir, *, today)` compares the profile's confirmed answers with `profile.snapshot.json`, the baseline the previous run wrote. It returns `{changed: [], entries: 0, error: 'no profile.md'}` without a profile, and `{…, error: 'profile does not validate: …', exit: 2}` without writing anything when `profile.blocking` reports problems — the subset of `validate` that makes a baseline meaningless (unreadable file, missing, unanswered or confirmed-`unknown` answer, bad status, unknown key). An answer still `proposed`, and an attestation written early, are reported by `profile validate` but do not stop a rescan: neither can move the snapshot. Missing `confirmed_by`/`confirmed_at` once all fifteen are confirmed does block, since the baseline would then rest on a profile no human has signed. A confirmed `unknown` (value null) also keeps blocking, decided 2026-09-22: a baseline over a dimension nobody can answer would put every later drift verdict on top of a hole (SPEC-001). A `profile.snapshot.json` that cannot be parsed returns `{…, error: 'profile.snapshot.json is unreadable: …', exit: 1}` and is never overwritten — the baseline is refused by name, not silently reset.
...
| BEH-310 a dimension put back to proposed keeps its last confirmed value: no change, no entry, snapshot untouched | accepted | `tests/test_rescan.py::test_re_proposing_a_confirmed_answer_neither_moves_the_snapshot_nor_files_an_entry` |
| BEH-037 a profile with an unanswered dimension returns exit 2 with the problems and writes no snapshot | accepted | `tests/test_rescan.py::test_invalid_profile_is_refused_with_exit_2` |
...
| 2026-09-22 | Gate moved from `profile.validate` to `profile.blocking`; a `proposed` answer no longer refuses a rescan; BEH-037 retitled (it pins the unanswered case, not every invalid profile) | SPEC-001 split the report from the gate |
| 2026-09-22 | A dimension that is not `confirmed` now carries its last confirmed value forward instead of reading as `null`; BEH-310 added (accepted) | Re-proposing a confirmed answer filed a false major `regime-gone` and wiped the baseline value (Principle 4) |
