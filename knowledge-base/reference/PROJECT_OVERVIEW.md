...
| Record, surface, delegate | `check` and `rescan` append to `pending.jsonl` and stop; only a human `resolve` moves anything. Both files are append-only and replayed | `compliance_register/pending.py:1-4`, `compliance_register/check.py:63-73`, `compliance_register/rescan.py:53-56` |
